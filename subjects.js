...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("https://smart-prep-sxdk.onrender.com/get-subjects")
        ...
        .then(data => {
            ...
            if (data.subjects && data.subjects.length > 0) {
                data.subjects.forEach(subject => {
                    ...
                    li.addEventListener("click", () => fetchTopics(subject));
...
async function fetchTopics(subject) {
    ...
    try {
        const response = await fetch(`https://smart-prep-sxdk.onrender.com/get-topics?state=${encodeURIComponent(state)}&curriculum=${encodeURIComponent(curriculum)}&grade=${encodeURIComponent(grade)}&subject=${encodeURIComponent(subject)}`);
